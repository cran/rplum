...
#' @param rounded Rounding of years. Default is to round to 1 digit, \code{rounded=1}.
...
#' @param pb.lim Limits of the 210Pb axis, if relevant. Calculated automatically by default, assuming no values can go below 0.
...
# Plum-specific
  ...
  if(set$isplum) {
     oldpar <- par(mar=c(3,3,1,3)) # to make space for righthand Pb axis
      on.exit(par(oldpar))         	
  } else {
